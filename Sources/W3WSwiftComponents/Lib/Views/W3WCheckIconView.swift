...
public class W3WCheckIconView: W3WDrawingView {
  ...
  func checkIcon(centre: CGPoint, radius:CGFloat, colour: UIColor, weight: CGFloat? = nil) {
    ...
    let path = UIBezierPath(arcCenter: centre, radius: radius, startAngle: 0.0, endAngle: 2.0 * .pi, clockwise: false)
        
    let checkBottom = CGPoint(x: centre.x - radius * (3.0 / 12.0), y: centre.y + radius * (5.0 / 12.0))
    let checkRight  = CGPoint(x: centre.x + radius * (6.0 / 12.0), y: centre.y - radius * (4.5 / 12.0))
    let checkLeft   = CGPoint(x: centre.x - radius * (6.0 / 12.0), y: centre.y + radius * (2.0 / 12.0))
